...
export default function TaskList() {
  ...
  // Atualizar tarefa
  const handleUpdateTask = (id: string, data: { title: string; description?: string }) => {
  ...
  return (
    <div className="max-w-lg mx-auto p-6 mt-8 bg-gray-100 dark:bg-gray-800 rounded-2xl shadow-lg transition-all duration-300">
      <h1 className="text-2xl font-semibold text-center text-gray-800 dark:text-gray-100 mb-4">
        Minhas tarefas
      </h1>

      <TaskForm
        onAdd={(title, description) => handleAdd(title, description)}
        onUpdate={(task) =>
        ...
        editing={
          editingTask
            ? tasks.find((t) => t.id === editingTask) ?? null
            : null
        }
      />
